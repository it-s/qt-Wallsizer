...
DropZone {
    ...
    onEntered: {
        if(message.isShown())message.hide();
        if(acceptable)indicator.show("ACCEPT");
        else indicator.show("REJECT");
        if(reason==1)message.show(qsTr("Selection contains non image items.\nOnly JPEG and PNG are accepted."));
        if(reason==2)message.show(qsTr("The converter does not support more then 12 items at a time."));
    }
    onLeft: {
        indicator.hide();
    }
